...
| `AddressAutocomplete` + `searchAddress` | A plain `.input-field` input backed by free OpenStreetMap/Nominatim search - type an address, pick a suggestion, the field fills with a clean single-line address (no Google Places API key/billing). Renders its own suggestions dropdown and the attribution line Nominatim's usage policy requires; no label of its own, compose it inside a consumer's own field wrapper. `searchAddress(query, {countryCodes, userAgent, limit})` is also exported standalone - `countryCodes` (e.g. `"ca"`) restricts results, `userAgent` identifies the calling app (both optional; browsers block a fetch call from actually setting the User-Agent header and send the page's Referer instead, which the policy also accepts). Self-rate-limits to Nominatim's 1 request/second cap internally, so a consumer's own debounce doesn't have to be tuned around it. `searchingLabel`/`attributionLabel` override the two fixed strings, e.g. for a bilingual consumer. Ported from `otoshi-landingpage`'s own `AddressAutocomplete`/`nominatim.ts` (itself from `Reel-FishR-React-Native`), generalized here: that original hardcoded `countrycodes=ca` and one app's own User-Agent and autofilled a registration form's separate City/Province/Postal Code fields from the suggestion's structured fields - this version leaves country/User-Agent to the caller and returns the full `AddressSuggestion` (`street`/`city`/`province`/`postalCode`/`lat`/`lng`/`countryCode`) for the caller to use however its own form is shaped, since `timesheet-payroll-system`/`pmg-intranet`'s address fields are a single free-text line (or two), not four structured ones. |
